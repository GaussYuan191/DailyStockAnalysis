...
## [Unreleased]
- [修复] LLM 分析调用缺少默认超时导致云服务器部署时 litellm 超时失败；`generation_config` 新增 120 秒默认超时，支持通过 `LLM_TIMEOUT_SEC` 环境变量覆盖。
- [修复] `requirements.txt` 缺少 `orjson` 依赖（litellm 响应解析需要），Docker 镜像运行时报 `No module named 'orjson'`。
- [改进] `docker-compose.yml` 默认使用 `network_mode: host` 并配置 DNS，解决云服务器上 Docker 桥接网络无法访问外部 API 的问题。
...
- [改进] AkShare 筹码分布调用 `stock_cyq_em` 增加超时保护（复用 `_akshare_call_with_timeout`），并在失败时提示配置 `TUSHARE_TOKEN` 启用 TushareFetcher 作为独立兜底。

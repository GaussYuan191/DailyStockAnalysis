...
import { BarChart3, Check, SlidersHorizontal, Upload, X } from 'lucide-react';
...
import { ApiErrorAlert, BatchImportDialog, Button, Drawer, EmptyState, InlineAlert } from '../components/common';
...
const HomePage: React.FC = () => {
  ...
  const existingStockCodes = useMemo(
    () => new Set(stockBarItems.map((item) => item.stockCode)),
    [stockBarItems],
  );
  ...
  return (
    ...
    >
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-2.5 md:flex-row md:items-center">
            ...
            <div className="flex min-w-0 flex-shrink-0 items-center gap-2.5">
              ...
              <button
                ...
                disabled={isAnalyzing}
                className="flex h-10 flex-1 items-center justify-center gap-1.5 whitespace-nowrap rounded-xl border border-subtle bg-surface/60 px-3 text-xs font-medium text-secondary-text transition-colors hover:border-subtle-hover hover:text-foreground disabled:cursor-not-allowed disabled:opacity-60 md:flex-none"
              >
      ...
      <BatchImportDialog
        ...
        existingStockCodes={existingStockCodes}
        onSubmitComplete={(count, skipped) => {
          setBatchImportNotice(
            skipped > 0
              ? t('home.batchImportSuccessWithSkipped', { count, skipped })
              : t('home.batchImportSuccess', { count }),
          );
        }}
        onError={(message) => {
          setBatchImportNotice(message);
        }}
      />

      {batchImportNotice ? (
        <div className="fixed bottom-4 right-4 z-[300] max-w-sm rounded-xl border border-subtle bg-elevated px-4 py-3 text-sm text-foreground shadow-2xl animate-in fade-in slide-in-from-bottom duration-300">
          <div className="flex items-start justify-between gap-3">
            <p>{batchImportNotice}</p>
            <button
              type="button"
              onClick={() => setBatchImportNotice(null)}
              className="flex-shrink-0 text-muted-text hover:text-foreground"
              aria-label={t('common.close')}
            >
              <X className="h-4 w-4" />
            </button>

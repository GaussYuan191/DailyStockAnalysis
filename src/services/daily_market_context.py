...
# Maximum lock-wait polling attempts.  With defaults (0.5s initial, 1.5x
# backoff, 5s cap) this yields ~55s total wait — enough for a concurrent
# market-review generation to finish, without blocking the analysis pipeline
# for minutes when the other process is slow or has failed.
# Override via env: MARKET_REVIEW_LOCK_WAIT_MAX_ATTEMPTS=20
_MARKET_REVIEW_LOCK_WAIT_MAX_ATTEMPTS = int(
    os.environ.get("MARKET_REVIEW_LOCK_WAIT_MAX_ATTEMPTS", "15")
)

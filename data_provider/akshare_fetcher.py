...
class AkshareFetcher(BaseFetcher):
    ...
    def get_chip_distribution(self, stock_code: str) -> Optional[ChipDistribution]:
        ...
        try:
            ...
            # 使用超时保护，防止东方财富接口挂起时阻塞分析流程
            df = _akshare_call_with_timeout(
                ak.stock_cyq_em,
                symbol=stock_code,
                timeout=self._history_call_timeout,
                call_name="stock_cyq_em",
            )
            ...
            # 筹码分布仅依赖东方财富接口（stock_cyq_em），当东方财富被限流时全部失败
            # 配置 TUSHARE_TOKEN 可启用 TushareFetcher 作为独立兜底数据源
            logger.warning(
                "[筹码分布] %s 东方财富接口失败，如需兜底请配置 TUSHARE_TOKEN 启用 TushareFetcher",
                stock_code,
            )
            return None

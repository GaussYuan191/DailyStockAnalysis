...
def _disable_efinance_urllib3_retries() -> None:
    """Disable urllib3 internal retries on the efinance shared session.

    When Eastmoney hosts reject connections (IP-level blocking, rate limiting),
    urllib3's default retry policy retries 3 times with backoff, adding ~4-5s
    of latency before the error surfaces.  Since EfinanceFetcher already has
    its own ``@retry`` decorator (capped at 1 attempt) and the DataFetcherManager
    provides multi-source failover, the urllib3-level retries are pure overhead.

    This helper mounts an ``HTTPAdapter`` with ``Retry(total=0)`` on the efinance
    shared session so that connection failures surface immediately, letting the
    fallback chain kick in without delay.
    """
    try:
        import efinance as ef
        from urllib3.util.retry import Retry

        session = getattr(ef.shared, "session", None)
        if session is None:
            return

        if getattr(session, "_dsa_no_retry_mounted", False):
            return

        no_retry = requests.adapters.HTTPAdapter(
            max_retries=Retry(total=0, backoff_factor=0)
        )
        session.mount("https://", no_retry)
        session.mount("http://", no_retry)
        session._dsa_no_retry_mounted = True
        logger.debug("已禁用 efinance session 的 urllib3 重试")
    except Exception as exc:
        logger.debug("禁用 efinance urllib3 重试失败（非致命）: %s", exc)
...
class EfinanceFetcher(BaseFetcher):
    ...
    priority = int(os.getenv("EFINANCE_PRIORITY", "1"))  # 东财 HTTP 接口有限流风险，降至 P1（PytdxFetcher/TencentFetcher P0 优先）
    ...
    def __init__(self, sleep_min: float = 1.5, sleep_max: float = 3.0):
        ...
        # 禁用 efinance session 的 urllib3 内部重试，加速连接失败后的数据源切换
        _disable_efinance_urllib3_retries()
        # 东财补丁开启才执行打补丁操作

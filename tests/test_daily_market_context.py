...
def test_get_context_skips_generation_when_market_review_lock_is_held() -> None:
    ...
    # With defaults (0.5s initial, 1.5x backoff, 5s cap, 15 attempts) the
    # total wait is ~54s.  Assert a lower bound that tolerates minor
    # backoff tuning while still proving the wait is substantial.
    assert total_wait_seconds > 40
